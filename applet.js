...
DimManager.prototype = {
	...
	_setSystemDimTimeout: function() {
		...
		if(this._dimTimeout > 0) {
			//session idle time should be smaller than display dim
			this.settingsManager.setSystemProperty('org.gnome.desktop.session idle-delay', Math.round(this._dimTimeout/3)*2);
		} else {
			//unless screen should be always on
			this.settingsManager.setSystemProperty('org.gnome.desktop.session idle-delay', this._dimTimeout);
		}
	}
}
